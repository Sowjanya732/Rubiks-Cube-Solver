...
function solveCube() {
  ...
  const fullSequence = steps.join('');
  const moveCount = fullSequence.match(/[A-Za-z]/g)?.length || 0;

  // Update info panel for LBL
  const el = document.getElementById('solutionMoves');
  if (el) el.textContent = `LBL (${moveCount}): ${fullSequence}`;

	// Label solve method
  const methodLabel = document.getElementById("solveMethod");
  if (methodLabel) methodLabel.textContent = "Solving method: LBL";

  const stepMoves = document.getElementById("stepMoves");
  if (stepMoves) stepMoves.innerHTML = ""; // clear previous steps

  // Display each step name + moves
  for (let i = 0; i < steps.length; i++) {
	const line = document.createElement("div");
	line.innerHTML = `<strong>${stepName[i]}:</strong> ${steps[i]}`;
	stepMoves.appendChild(line);
  }
